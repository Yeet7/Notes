...
\ex{2: Graph the exponential function: $f(x) = 2^{x}$}{}
	\begin{figure}[ht]
		...
		\caption{$f(x) = 2^{x}$}
		\label{101}
		\includegraphics[scale=0.25]{2^x.jpg}
	\end{figure}
\nt{Properties of the Exponential Function: $f(x) = a^{x}$, where $a>1$
	...
	\end{enumerate}}
\ex{3: Graph the exponential function: $f(x)=(\frac{1}{2})^{2}$}{s}
\begin{figure}[ht]
	\centering
	\caption{$f(x)=(\frac{1}{2})^{2}$}
	\label{102}
	\includegraphics[scale=0.25]{0.5^x.jpg}
\end{figure}
...
\begin{figure}[ht]
	...
	\caption{$f(x) = 2^{-x}-3$}
	\label{103}
	\includegraphics[scale=0.25]{-2^x-3.jpg}
\end{figure}
...
\ex{5: Find the domain of each logarithmic function:}{
	\begin{enumerate}
		...
		\begin{itemize}
			\item $x>-3$ or $(-3,\infty)$
		\end{itemize}
		\item $g$($x$) = $\log_{b}(\frac{1+x}{1-x}) \rightarrow \frac{1+x}{1-x}>0$
		\begin{itemize}
...
\begin{figure}
	...
	\caption{$\log_{10}x$}
	\label{log_10(x)}
	\includegraphics[scale=0.25]{log_10 x.jpg}
\end{figure}
...
\begin{enumerate}
	\item Suppose that a concentration in the blood of 0.04 results in a 10\% risk (R=10) of an accident. Find the constant k in the equation. Graph $R=6e^{kx}$ using the k value.
	\begin{itemize}
		\item do stuff so that k=20.62. She literally used her calc
	\end{itemize}
	...
	\begin{itemize}
		\item didn't do this one either. aparently D is the most important.
	\end{itemize}
	\item If the law asserts that anyone with a risk of having an accidnet of 20\% or more should not have driving privileges, at what concentration of alcohol in the blood should a driver be arrested and charged with a DUI?
	\begin{itemize}
		\item $20=6e^{12.77x}$
		\item $\frac{10}{3}=e^{12.77x}$
		\item $\ln(\frac{10}{3})=12.77x$
		\item $x=0.94$
	\end{itemize}
\end{enumerate}}
...
\ex{9: Use a graphing utility to graph the following:}{
	\begin{enumerate}
		\item $y=\log_{2}x$
		\item make graphhhhhhhhhhhhhhhhhhhhhh
	\end{enumerate}
}
